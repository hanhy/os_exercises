...
```
>
	flag.s和peterson.s并没有实现锁机制。创造两个线程即设置-i为2，分别运行flag.s和peterson.s观察发现，我们创建的两个线程都进入了临界区，所以没有锁机制的实现。
	test-and-set.s实现了锁机制，由于其采用的是原子操作，不能被中断打断，所以不能被其他线程扰乱，实现了互斥。
	test-and-test-and-set.s、ticket.s也都实现了锁机制。
```
